...
Dynamic plugin records are special as well. Relay matches `[[plugins.dynamic]]`
records by the plugin ID declared in their manifests. A higher-precedence record
with the same ID replaces the lower-precedence record entirely, including its
manifest reference and optional `config`. This lets system configuration select
the approved dynamic plugin package and configuration without inheriting values
from the user or explicit file. Declare a plugin ID at most once in each
individual `plugins.toml` file.
