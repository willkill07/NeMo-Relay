...
The gateway does not serve Google's Gemini or Vertex API. A Gemini model is
therefore skipped with `unserviceable-api`: pi keeps using the normal Google
endpoint, but its model request does not reach Relay. This means Relay cannot
record the LLM call or apply LLM request rules to it.
...
then configure Pi to use that compatible API. A Gemini endpoint cannot use an
OpenAI or Anthropic gateway route because the request formats differ. Relay's
Gemini codec can translate Gemini data in
[application integrations](/integrate-into-frameworks/provider-codecs), but it
does not add a gateway route.
