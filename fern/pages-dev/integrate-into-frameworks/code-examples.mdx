...
def framework_tool_started(name: str, args: dict[str, object]) -> nemo_relay.ToolCallHandle:
    return nemo_relay.tools.call(name, args)

def framework_tool_finished(handle: nemo_relay.ToolCallHandle, result: dict[str, object]) -> None:
    nemo_relay.tools.call_end(handle, nemo_relay.ToolExecutionResult(result))

def framework_llm_started(provider: str, payload: dict[str, object]) -> nemo_relay.LLMCallHandle:
    request = LLMRequest({}, payload)
    return nemo_relay.llm.call(provider, request, model_name=payload.get("model"))

def framework_llm_finished(handle: nemo_relay.LLMCallHandle, response: dict[str, object]) -> None:
