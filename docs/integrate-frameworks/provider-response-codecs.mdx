...
- A managed LLM boundary from [Wrap LLM Calls](/integrate-frameworks/wrap-llm-calls).
...
- Use [Provider Codecs](/integrate-frameworks/provider-codecs) for request-side provider codecs and full request/response examples.
- Use [Wrap LLM Calls](/integrate-frameworks/wrap-llm-calls) to add the managed LLM boundary first.
- Use [Observability](/plugins/observability/about) after annotations are visible in local subscribers.
