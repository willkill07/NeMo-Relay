...
For provider-native LLM payload normalization, use [Provider Codecs](/integrate-frameworks/provider-codecs).
...
Use this page for typed value codecs. Use [Provider Codecs](/integrate-frameworks/provider-codecs) when middleware needs normalized LLM messages, tools, model names, generation parameters, or provider response annotations.
...
- Use [Provider Codecs](/integrate-frameworks/provider-codecs) when provider payloads need normalized request or response annotations.
- Use [Handle Non-Serializable Data](/integrate-frameworks/non-serializable-data) when framework objects cannot pass through JSON payloads.
- Use [Integrate into Frameworks Code Examples](/integrate-frameworks/code-examples) for explicit fallback APIs.
