...
- [Native Dynamic Plugins](/build-plugins/native-dynamic-plugins) documents trusted in-process shared-library plugins and the stable native ABI.
- [Build a Rust Native Plugin](/build-plugins/rust-native-plugin-example) shows the high-level Rust `nemo-relay-plugin` SDK path.
- [Worker Dynamic Plugins](/build-plugins/worker-dynamic-plugins) documents trusted out-of-process plugin workers using the stable `grpc-v1` protocol.
- [Build a Python Worker Plugin](/build-plugins/python-worker-plugin-example) shows the Python `nemo-relay-plugin` package.
- [gRPC Worker Protocol](/build-plugins/grpc-worker-protocol) describes the high-level `grpc-v1` worker API.
