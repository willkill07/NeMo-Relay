...
`relay-plugin.toml`. It reads the schema without loading a native library or
starting a worker. `nemo-relay plugins add`, `nemo-relay plugins validate`, and
...
object at the schema root. Only same-document fragment references beginning
with `#`, such as `#/$defs/auth`, are supported. Relative files, network URLs,
and file URLs in `$ref` are not supported. Draft 2020-12 dynamic references
and anchors (`$dynamicRef` and `$dynamicAnchor`) are not supported. The editor provides structured
controls for booleans, strings, numbers, string enums, nested objects, and
string-valued maps. It uses a raw JSON control for arrays, unions, and other
complex values, then validates the result against the complete schema before
saving.
