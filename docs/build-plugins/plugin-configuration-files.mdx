...
recursive reference cycles, relative files, network URLs, and file URLs in
`$ref` are not supported. The editor provides structured controls for
booleans, strings, numbers, string enums, nested objects, and string-valued
maps. It uses a raw JSON control for arrays, unions, and other complex values,
then validates the result against the complete schema before saving.
