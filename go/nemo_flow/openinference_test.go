...
func TestNewOpenInferenceConfigDefaults(t *testing.T) {
	...
	if config.ServiceName != "nemo-flow" {
		t.Fatalf("expected default service name nemo-flow, got %q", config.ServiceName)
	}
	if config.InstrumentationScope != "nemo-flow-openinference" {
